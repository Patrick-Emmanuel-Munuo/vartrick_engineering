...
const handleBulkSubmit = async () => {
  try {
    ...
    const payload = filter_users.map(user => ({
    ...
    const response = await props.application.post({
      ...
      body: { 
        table: "users",
        data: payload  // send only necessary fields 
      },
    });
    ...
    if (response?.message && Array.isArray(response.message)) {
      ...
      dispatch({ notification: "No response from server or invalid format" });
    }
  } catch (error) {
    dispatch({ loading: false });
...
const filter_users = React.useMemo(() => {
  ...
  return users.map((user, index) => ({
        ...
        status: "active",
        password: encrypt({ password: "Bmh@1234" }).encrypted || ""
  }));
}, [state.file_data]);
